  ...
  <div>
      ...
      <div class="overflow-y-auto px-2 pt-2" style="max-height: 75vh">
        <v-row v-if="invoice_doc" class="px-1 py-0">
          ...
          <v-col cols="5" v-if="diff_payment < 0 && !invoice_doc.is_return">
            <v-text-field outlined color="indigo" :label="frappe._('Credit Change')" background-color="white" hide-details
              :value="formtCurrency(credit_change)" disabled :prefix="invoice_doc.currency" dense></v-text-field>
          </v-col>
        </v-row>
        <v-divider></v-divider>
        ...
        ">
          ...
          <v-col cols="5">
            <v-text-field dense outlined color="indigo" :label="frappe._('You can redeem upto')" background-color="white"
              hide-details :value="formtCurrency(available_pioints_amount)" :prefix="invoice_doc.currency"
        ...
        <v-row class="px-1 py-0" justify="center" align="start">
          <v-col cols="6">
            ...
            " v-model="is_credit_sale" flat :label="frappe._('Is Credit Sale')" class="my-0 py-0"></v-switch>

            <v-switch v-if="invoice_doc.is_return && pos_profile.use_cashback" v-model="is_cashback" flat
              :label="frappe._('Is Cashback')" class="my-0 py-0"></v-switch>
          </v-col>
          <v-col cols="6">
            <v-menu v-if="is_credit_sale" ref="date_menu" v-model="date_menu" :close-on-content-click="false"
              transition="scale-transition">
              <template v-slot:activator="{ on, attrs }">
        ...
        <v-row>
          <v-col cols="12" md="6">
            <v-switch v-if="!invoice_doc.is_return && pos_profile.use_customer_credit" v-model="redeem_customer_credit"
              flat :label="frappe._('Use Customer Credit')" class="my-0 py-0"
    ...
    <v-card flat class="cards mb-0 mt-3 py-0">
      <v-row align="start" no-gutters>
        ...
        <v-col cols="12">
          <v-btn block class="mt-2" large color="primary" dark @click="submit" :disabled="vaildatPayment">
            {{ __("Submit Payments") }}
          </v-btn>
        </v-col>
      </v-row>
